    ...
    async def send_audio(self, audio_chunk: bytes, sample_rate: int = None, encoding: str = None):
        ...
        try:
            ...
            if self._greeting_transport_guard_active:
                # Do not destroy caller audio during the greeting.  Greeting
                # barge-in is independently blocked below/in the provider event
                # handler, while the engine's TTS gating protects the greeting
                # lifecycle.  Replacing these samples with zeroes caused speech
                # beginning near the end of the greeting (for example,
                # "I'm Rufus") to be permanently lost.
                if not self._greeting_guard_silence_logged:
                    logger.info(
                        "Preserving caller input during protected greeting",
                        call_id=self._call_id,
                    )
            ...
            if (
                ...
                if (getattr(self, "_context_name", None) == "operator_zero_incoming"
                    ...
                    raise RuntimeError(
                        ...
                        "Do not repeat it or transfer yet. A business name or broad reason is enough; offer voicemail if neither is supplied."
                    )
